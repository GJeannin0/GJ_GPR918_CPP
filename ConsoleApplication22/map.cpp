...
template<typename T> bool Map::gameObjectOfTypeExistAt(Vector2D position)
{
	if (map[position.x][position.y] == nullptr)
	{									// un ptr nul sur une case donne false
		return false;
	}
	T * gameObject = dynamic_cast<T*>(map[position.x][position.y]);
	return gameObject != nullptr;													// un objet retourne un true
}
